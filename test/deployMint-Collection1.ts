...
describe("CollageOfMyself Contract", function () {
  ...
  it("It should activate and mint 5 nfts", async function () {
    ...
    expect(await collageOfMyself.balanceOf(owner.address)).to.equal(5n);
  });
  it("It should mit 15 more nfts", async function () {
    ...
    expect(await collageOfMyself.balanceOf(owner.address)).to.equal(20n);
  });
  it("It should set a public username", async function () {
    ...
    await startMinting.wait();
    await collageOfMyself.mint(5);
    const publicUsername = await collageOfMyself.publicUsernameOfOwner(
    ...
    const publicUsername2 = await collageOfMyself.publicUsernameOfOwner(
      "0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266",
    );
